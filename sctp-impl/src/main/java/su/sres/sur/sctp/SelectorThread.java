		...
		for (Server srv : this.management.servers) {
			...
			if (srvv.getIpChannel() == serverSocketChannel) { // we have found a server
				for (SocketAddress sockAdd : peerAddresses) {
					...
					Iterator<FastMap.Entry<String, Association>> iter = associations.entrySet().iterator();
					while(iter.hasNext() && !provisioned) {
					    AssociationImpl association = (AssociationImpl) iter.next().getValue();
